...
export default function ResortsData({ resorts, limit }) {
    ...
    const handleClick = (id) => {
        ...
        naviagte(`/resort-detail/${id}?location=${searchInfo.location}&checkIn=${searchInfo.checkIn}&checkOut=${searchInfo.checkOut}&adult=${searchInfo.adult}&children=${searchInfo.children}`)
    }
                    ...
                    <Col md={12} className='m-auto'>
                        <Card className='m-auto p-3 border-0 border-start'>
                            <Card.Body>
                                ...
                                <Card.Title > ## {resorts.totalRecords}+ stay in this area </Card.Title> <hr />
                            ...
                            <div className='row'>
                                <div className='col-2'>
                                    <button className='btn btn-primary' disabled={resorts.pageNo === 1} onClick={handlePrev}><FaArrowLeft /> Previous </button>
                                </div>
                                <div className='offset-4 col-2'>
                                    {resorts.pageNo}
                                </div>
                                <div className='offset-2 col-2'>
                                    <button className='btn btn-primary' onClick={handleNext} disabled={resorts.totalPages === resorts.pageNo}> Next <FaArrowRight /> </button>
                                </div>
                            </div>
                        </Card>
                    </Col>
